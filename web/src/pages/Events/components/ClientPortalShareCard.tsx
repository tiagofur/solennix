...
export const ClientPortalShareCard: React.FC<Props> = ({ eventId }) => {
  ...
  const handleGenerate = async () => {
    try {
      ...
      addToast(t("client_portal_share.toast_generate"), "success");
    } catch (err) {
  ...
  const handleRotate = async () => {
    if (
      !window.confirm(
        t("client_portal_share.confirm_rotate"),
      )
    ) {
      return;
    }
    try {
      await createOrRotate.mutateAsync({});
      addToast(t("client_portal_share.toast_rotate"), "success");
    } catch (err) {
  ...
  const handleRevoke = async () => {
    if (
      !window.confirm(
        t("client_portal_share.confirm_revoke"),
      )
    ) {
      return;
    }
    try {
      ...
      addToast(t("client_portal_share.toast_revoke"), "info");
    } catch (err) {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      addToast(t("client_portal_share.toast_copy"), "success");
    } catch (err) {
  ...
  const handleShareWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      t("client_portal_share.share_message", { url: link.url }),
    );
    ...
    <section className="bg-card rounded-2xl border border-border shadow-sm p-6">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          <h3 className="text-base font-semibold text-text">
            {t("client_portal_share.title")}
          </h3>
          <p className="text-sm text-text-secondary">
            {t("client_portal_share.description")}
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {copied ? (
                <>
                  ...
                  {t("client_portal_share.copied")}
                </>
              ) : (
                <>
                  <Copy className="h-4 w-4" aria-hidden="true" />
                  {t("client_portal_share.copy_link")}
                </>
              )}
            </button>
            ...
            >
              ...
              {t("client_portal_share.share_whatsapp")}
            </button>
            ...
            <button
              ...
              title={t("client_portal_share.rotate_title")}
            >
              <RefreshCw className="h-4 w-4" aria-hidden="true" />
              {t("client_portal_share.rotate")}
            </button>
        ...
        >
          ...
          {t("client_portal_share.generate")}
        </button>
      )}
    </section>
  );
